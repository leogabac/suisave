...
If a job references several remotes, `--push` can run across all of them in one run.
...
The aliases exist for backwards compatibility, and because some people think in terms of transfer direction, while others think in terms of which side is the source of truth.
